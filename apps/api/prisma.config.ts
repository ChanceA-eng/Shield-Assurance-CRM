...
if (process.env.DATABASE_URL && process.env.PRISMA_SKIP_URL_NORMALIZE !== '1') {
  process.env.DATABASE_URL = buildPrismaDatabaseUrl();
}

export default {
  schema: path.resolve(repoRoot, 'apps/api/prisma/schema.prisma'),
  migrations: {
    path: path.resolve(repoRoot, 'apps/api/prisma/migrations'),
  },
};